...
export default class GitHubFileURL {
  ...
  assign(...replacements: Partial<GitHubFileURL>[]): this {
  ...
  private disambiguateReference(ambiguousReference: string[]): {
    ...
    const branch: string = ambiguousReference[0]!;
    // History pages might use search parameters
    const filePathFromSearch: string = this.searchParams.getAll('path[]').join('/');
    ...
    const currentBranch: string | undefined = getCurrentGitRef();
    const currentBranchSections: string[] | undefined = currentBranch?.split('/');
    const teste: 'asd' = 'asd';
