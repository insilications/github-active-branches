...
const escapeKey = (...keys: (string | number)[]): string =>
...
export const v3uncached = async (
  ...
  const personalToken: string = configManager.get('GITHUB_TOKEN');
  ...
  const textContent: string = await response.text();
  const apiResponse: RestResponse | { textContent: string } = json
    ? (JSON.parse(textContent) as RestResponse)
    : { textContent };
...
const v4uncached = async (
  ...
  const apiResponse: GraphQLResponse = (await response.json()) as GraphQLResponse;
...
function getError(apiResponse: JsonObject): RefinedGitHubAPIError {
  const personalToken: string = configManager.get('GITHUB_TOKEN');
